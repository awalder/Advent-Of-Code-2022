...
struct Monkey
{
    std::vector<int> items; // value == worrylevel
...
int main()
{
    ...
    while(std::getline(file, line))
    {
        ...
        else if(line.starts_with("  Operation:"))
        {
            ...
            monkey.operation = [op = std::string(rng.at(0)),
                                rhs = std::string(rng.at(1))](int old) -> int {
                int b = rhs == "old" ? old : std::stoi(rhs);
                int res = op == "*" ? old * b : old + b;
                return std::floor(res / 3);
            };
        }
        else if(line.starts_with("  Test"))
        {
            ...
            monkey.test = [divisor = std::stoi(rng.at(3))](int value) -> bool {
                // fmt::print(
                //         "TEST: divide {} % {} = {}\n",
                //         value,
                //         divisor,
    ...
    for(int i = 0; i < 20; ++i)
    {
        ...
        for(auto& monkey : monkeys)
        {
            ...
            ranges::for_each(rng, [&monkeys, &monkey](int item) {
    ...
    auto res = ranges::accumulate(
            counts | rv::reverse | rv::take(2), 1, std::multiplies<int>{});
